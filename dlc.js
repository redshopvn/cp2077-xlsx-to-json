...
const processDLCSheet = (sheet) => {
    ...
    listOfFiles.forEach(file => {
        ...
        entries.forEach(entry => {
            if((entry.stringId && entry.stringId.length > 0) && (entry.primaryKey && entry.primaryKey.length > 0) && entry.femaleVariant && entry.femaleVariant.length > 0) {
                const found = jsonWorkbook.find(r => r.STRING === entry.stringId || r.STRING === entry.primaryKey);
                if (found) {
